const fs = require('fs');
const { readJson, writeJsonByKey } = require('./weread-service');
const {
  buildPersonaPrompt,
  buildMbtiPrompt,
  buildCocoonPrompt,
...
async function loadAllData(apiKey) {
  const [raw, cardsData, classified, taxonomy] = await Promise.all([
    readJson(apiKey, 'weread-data.json', { fetchedAt: '', totalBooks: 0, books: [] }),
    readJson(apiKey, 'cards.json', { cards: [], taxonomy: [] }),
    readJson(apiKey, 'classified.json', { notes: [], stats: {} }),
    readJson(apiKey, 'taxonomy.json', { categories: [] }),
  ]);
...
async function readCache(apiKey) {
  return readJson(apiKey, 'reports.json', { generatedAt: '', reports: {} });
}
...
async function writeCacheEntry(apiKey, reportId, content, dataHash) {
  ...
  await writeJsonByKey(apiKey, 'reports.json', cache);
}
