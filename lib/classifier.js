...
function getConfig() {
  return {
    baseUrl: (process.env.LLM_EMBEDDING_BASE_URL || process.env.LLM_BASE_URL || DEFAULT_BASE_URL).replace(/\/+$/, ''),
...
async function getEmbeddings(texts, config) {
  ...
  for (let i = 0; i < texts.length; i += maxBatch) {
    batches.push({ start: i, texts: texts.slice(i, i + maxBatch) });
  }
    ...
    while (nextIdx < batches.length) {
      ...
      for (let retry = 0; retry < 3; retry++) {
        res = await fetch(`${baseUrl}/embeddings`, {
          ...
          body: JSON.stringify({ model: embeddingModel, input: batch.texts }),
        });
        if (res.ok) break;
      ...
      for (let j = 0; j < sorted.length; j++) {
        allEmbeddings[batch.start + j] = sorted[j].embedding;
      }
    }
  }
